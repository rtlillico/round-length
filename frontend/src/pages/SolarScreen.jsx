...
import { PASTURE_PARAMS, dateToDayOfYear, calcTempLAR, MAX_SOLAR_BY_MONTH } from '../lib/formula';
import {
...
function buildArrays(chartData, targetLeaves, pastureKey) {
  ...
  // NOTE: variable names are kept from the Temperature screen so the chart engine
  // is reused unchanged. Here they hold SOLAR data:
  //   larData / larP* = Solar factor (Card 1, right axis, 0–1)
  //   roundData / roundP50 / rndP* = Solar round length (Card 1, left axis, days)
  //   tMeanData = Radiation actual (Card 2);  tempPc.mean = Radiation percentile bands
  const larData   = new Array(N).fill(null);   // solar factor actual
  const larP10    = new Array(N).fill(null);   // solar factor percentiles
  ...
  const roundData = new Array(N).fill(null);   // solar round length
  ...
  const tMaxData  = new Array(N).fill(null);   // unused (Card 2 is a single series)
  const tMeanData = new Array(N).fill(null);   // radiation actual
  const tMinData  = new Array(N).fill(null);   // unused
  const A = () => new Array(N).fill(null);
  // Card-2 bands: only the 'mean' slot is used — it holds RADIATION percentiles.
  const tempPc = {
  ...
  // Solar LAR = temp LAR × solar factor — drives solar round length
  const actualSolarLARs = allActual.map(r => Number(r.temp_lar ?? 0) * (r.solar_factor != null ? Number(r.solar_factor) : 1));
  const actualIdx = {};
  for (let i = 0; i < allActual.length; i++) actualIdx[allActual[i].date?.slice(0, 10)] = i;

  const num = (v) => v != null ? Number(v) : null;
  // Per-day-of-year solar-LAR percentiles (for the solar round-length bands)
  const sfLARp10 = A(), sfLARp25 = A(), sfLARp50 = A(), sfLARp75 = A(), sfLARp90 = A();

  for (let i = 0; i < N; i++) {
    const doy  = dateToDayOfYear(new Date(dates[i] + 'T00:00:00Z'));
    const perc = percByDoy[doy] || {};
    // Card 1 metric A: solar factor percentiles (0–1)
    larP10[i] = num(perc.solar_p10); larP25[i] = num(perc.solar_p25); larP50[i] = num(perc.solar_p50);
    larP75[i] = num(perc.solar_p75); larP90[i] = num(perc.solar_p90);
    // Card 2: radiation percentile bands (only 'mean' slot used)
    tempPc.mean.p10[i] = num(perc.rad_p10); tempPc.mean.p25[i] = num(perc.rad_p25); tempPc.mean.p50[i] = num(perc.rad_p50); tempPc.mean.p75[i] = num(perc.rad_p75); tempPc.mean.p90[i] = num(perc.rad_p90);
    // Solar LAR percentile = temp-LAR(temp percentile) × solar-factor percentile
    const sfLAR = (tPx, sPx) => (tPx != null && sPx != null) ? calcTempLAR(Number(tPx), pastureKey) * Number(sPx) : null;
    sfLARp10[i] = sfLAR(perc.temp_p10, perc.solar_p10);
    sfLARp25[i] = sfLAR(perc.temp_p25, perc.solar_p25);
    sfLARp50[i] = sfLAR(perc.temp_p50, perc.solar_p50);
    sfLARp75[i] = sfLAR(perc.temp_p75, perc.solar_p75);
    sfLARp90[i] = sfLAR(perc.temp_p90, perc.solar_p90);
  }
  ...
  for (let i = 0; i <= TODAY; i++) {
    ...
    larData[i]   = row.solar_factor != null ? Number(row.solar_factor) : null; // solar factor actual
    tMeanData[i] = row.radiation    != null ? Number(row.radiation)    : null; // radiation actual

    let sum = 0, days = 0;
    for (let j = ai; j >= 0; j--) { sum += actualSolarLARs[j]; days++; if (sum >= targetLeaves) { roundData[i] = days; break; } }
    if (roundData[i] == null) roundData[i] = days;
  }

  // Solar round-length percentiles — accumulate the solar-LAR percentiles backward
  // over a full (wrapped) climatological year. Higher LAR → shorter round, so they
  // invert: round P10 (fast) from solar-LAR P90, round P90 (slow) from solar-LAR P10.
  ...
  for (let i = 0; i < N; i++) {
    rndP10[i] = roundAccum(sfLARp90, i);
    rndP25[i] = roundAccum(sfLARp75, i);
    rndP50[i] = roundAccum(sfLARp50, i);
    rndP75[i] = roundAccum(sfLARp25, i);
    rndP90[i] = roundAccum(sfLARp10, i);
    roundP50[i] = rndP50[i];
  }
...
// Radiation percentile-comparison colours (only the 'mean' slot is used on Solar)
const TPC_COL = {
  min:  { key: 'tMin',  label: 'Radiation', band: '#c47a12', actual: '#7a4a08', rgb: '196,122,18' },
  mean: { key: 'tMean', label: 'Radiation', band: '#c47a12', actual: '#7a4a08', rgb: '196,122,18' },
  max:  { key: 'tMax',  label: 'Radiation', band: '#c47a12', actual: '#7a4a08', rgb: '196,122,18' },
};
...
export default function SolarScreen({ scenario, chartData, loading, onNavigate, onGoToScenarios }) {
  ...
  // Solar "today" values
  const solarFactorToday = st?.solar_factor != null ? Number(st.solar_factor) : null;
  const radiationToday   = st?.radiation    != null ? Number(st.radiation)    : null;
  const maxSolarMonth    = MAX_SOLAR_BY_MONTH[new Date().getMonth()];
  ...
  // percentile card refs — chart 1 (Solar factor)
  ...
  const mkYR = () => ({ type: 'linear', position: 'right', ticks: { color: '#4aa8d8', font: { size: 9 }, maxTicksLimit: 5, callback: withUnit('SF')  }, grid: { display: false }, border: { display: false } });
  const mkYS = () => ({                                     ticks: { color: '#5a6f48', font: { size: 9 }, maxTicksLimit: 4, callback: withUnit('MJ')   }, grid: { color: 'rgba(0,0,0,0.04)' }, border: { display: false } });
  function mkYRZoom() {
    ...
    return { type: 'linear', position: 'right', min: 0, max, ticks: { color: '#4aa8d8', font: { size: 9 }, maxTicksLimit: 3, callback: withUnit('SF') }, grid: { display: false }, border: { display: false } };
  }
  function mkYRpc() {
    ...
    const rawMax = vals.length ? Math.max(...vals) : 0.15;
    const max = Math.ceil(rawMax * 20) / 20;
    return { type: 'linear', position: 'right', min: 0, max, ticks: { color: '#4aa8d8', font: { size: 9 }, maxTicksLimit: 4, callback: withUnit('SF') }, grid: { color: 'rgba(0,0,0,0.04)' }, border: { display: false } };
  }
  ...
  function mkYpcTemp(win) {
    ...
    const base = { type: 'linear', position: 'right', ticks: { color: '#4aa8d8', font: { size: 9 }, maxTicksLimit: 4, callback: withUnit('MJ') }, grid: { color: 'rgba(0,0,0,0.04)' }, border: { display: false } };
  ...
  function posOverlays() {
    ...
    if (showTpcRef.current) {
      ...
      const ft = (v) => v != null ? v.toFixed(1) + ' MJ' : '—';
      setCtrTpc(pds ? {
        dl: fmtDayFull(pds),
    ...
    setCtr2(ds ? { dl: fmtDayFull(ds), tMean: tMeanData[cDay] != null ? tMeanData[cDay].toFixed(1) + ' MJ' : '—' } : null);
      ...
      <div style={{ padding: '10px 10px 0' }}>
        ...
        <div style={styles.card}>
          <div style={{ fontSize: 18, fontWeight: 700, lineHeight: 1.3, display: 'flex', alignItems: 'center', flexWrap: 'wrap', marginBottom: 4 }}>
            ...
            {' '}
            <span style={{ color: '#c47a12' }}>Solar round length</span>
          ...
          {infoRL && (
            <div style={{ background: '#fff8ed', border: '1px solid rgba(196,122,18,0.3)', borderRadius: 8, padding: '10px 12px', marginBottom: 10, fontSize: 12, color: '#6b4a10', lineHeight: 1.6 }}>
              <div style={{ fontWeight: 600, color: '#c47a12', marginBottom: 6 }}>Solar round length</div>
              <div style={{ marginBottom: 8 }}>How many days it would take to complete one grazing round if growth were driven by temperature and sunlight together (no moisture limit). Calculated by summing daily Solar LAR (Temp LAR × Solar factor) backward from today until the target leaf stage is reached.</div>
              <pre style={{ fontSize: 10, background: 'rgba(196,122,18,0.08)', borderRadius: 6, padding: '6px 8px', margin: 0, whiteSpace: 'pre-wrap', fontFamily: 'monospace', color: '#7a4a08' }}>{`Solar round length = backward sum of (Temp LAR × Solar factor)\n  until sum ≥ target leaves (${scenario.target_leaves} leaves)`}</pre>
            </div>
          )}
          {infoLAR && (
            <div style={{ background: '#eef7fd', border: '1px solid rgba(74,168,216,0.3)', borderRadius: 8, padding: '10px 12px', marginBottom: 10, fontSize: 12, color: '#1a4a6b', lineHeight: 1.6 }}>
              <div style={{ fontWeight: 600, color: '#4aa8d8', marginBottom: 6 }}>Solar factor (0–1)</div>
              <div style={{ marginBottom: 8 }}>How much sunlight limits growth. It's the day's radiation as a fraction of the brightest typical day for the month, capped at 1. A value of 1 means light isn't limiting; lower values scale leaf growth down.</div>
              <pre style={{ fontSize: 10, background: 'rgba(74,168,216,0.08)', borderRadius: 6, padding: '6px 8px', margin: '0 0 8px', whiteSpace: 'pre-wrap', fontFamily: 'monospace', color: '#1a3a5a' }}>{`Solar factor = min(1, radiation / max_solar_for_month)`}</pre>
              {[
                { label: 'Max solar this month', value: `${maxSolarMonth} MJ/m²/day`,                          desc: 'brightest typical day' },
                { label: 'Radiation today',      value: radiationToday != null ? `${radiationToday.toFixed(1)} MJ/m²/day` : '—', desc: null },
                { label: 'Solar factor today',   value: solarFactorToday != null ? solarFactorToday.toFixed(3) : '—', desc: null },
          ...
          {!loading && (
            <>
              ...
              <div style={{ fontSize: 11, color: '#2d4a1e', marginTop: 6, background: '#f5fae8', border: '1px solid #cfe2b3', borderRadius: 6, overflow: 'hidden' }}>
                ...
                >
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '3px 12px' }}>
                    {[
                      { color: '#4aa8d8', dashed: false, label: 'Solar factor' },
                      { color: '#4aa8d8', dashed: true,  label: 'SF avg' },
                      { color: '#c47a12', dashed: false, label: 'Solar round length' },
                ...
                {expandCtr1 && (
                  <div style={{ padding: '0 10px 8px', borderTop: '1px solid #e0eecb', lineHeight: 1.7 }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2px 10px' }}>
                      {[
                        { color: '#4aa8d8', dashed: false, label: 'Solar factor',    value: ctr1?.lar },
                        { color: '#4aa8d8', dashed: true,  label: 'SF avg',     value: ctr1?.larAvg },
                        { color: '#c47a12', dashed: false, label: 'Solar round length', value: ctr1?.round },
              ...
              <ToggleBar show={visC1} onToggle={k => setVisC1(p => ({ ...p, [k]: !p[k] }))} items={[
                { key: 'tempLAR',   label: 'Solar factor',          color: '#4aa8d8' },
                { key: 'tempRound', label: 'Solar round length', color: '#c47a12' },
              ]} />
                ...
                {showPct && (
                  <div style={{ padding: '0 12px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, paddingTop: 10 }}>
                      {[
                        { key: 'lar',   label: 'Solar factor',          color: '#4aa8d8' },
                        { key: 'round', label: 'Solar round length', color: '#c47a12' },
                      ].map(({ key, label, color }) => (
                    ...
                    {infoPc && (
                      <div style={{ background: '#eef7fd', border: '1px solid rgba(74,168,216,0.3)', borderRadius: 8, padding: '10px 12px', margin: '8px 0 4px', fontSize: 12, color: '#1a4a6b', lineHeight: 1.6 }}>
                        {pcMetric === 'lar' ? (
                          <>
                            <div style={{ fontWeight: 600, color: '#4aa8d8', marginBottom: 6 }}>Solar factor — Leaf Appearance Rate</div>
                          ...
                          <>
                            <div style={{ fontWeight: 600, color: '#4aa8d8', marginBottom: 6 }}>Solar round length</div>
                    ...
                    <div style={{ fontSize: 11, color: '#2d4a1e', marginTop: 6, background: '#f0f7fd', border: '1px solid #c0daf0', borderRadius: 6, overflow: 'hidden' }}>
                      <div onClick={() => setExpandCtrPc(v => !v)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 10px', cursor: 'pointer', gap: 8 }}>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '3px 12px' }}>
                          {[
                            { color: '#1a4a7a', dashed: false, fill: null,                       label: 'Actual solar factor' },
                      ...
                      {expandCtrPc && (
                        <div style={{ padding: '0 10px 8px', borderTop: '1px solid #c0daf0', lineHeight: 1.7 }}>
                          ...
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2px 10px' }}>
                            {[
                              { color: '#1a4a7a', dashed: false, fill: null,                    label: 'Actual solar factor', value: ctrPc?.lar },
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: '#2d4a1e', marginTop: 6, background: '#f0f7fd', border: '1px solid #c0daf0', borderRadius: 6, overflow: 'hidden' }}>
                        <div onClick={() => setExpandCtrPc2(v => !v)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 10px', cursor: 'pointer', gap: 8 }}>
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '3px 12px' }}>
                            {[
                              { color: '#1a4a7a', dashed: false, fill: null,                       label: 'Actual solar round length' },
                              { color: '#4aa8d8', dashed: true,  fill: null,                       label: 'P50' },
                              { color: '#4aa8d8', dashed: false, fill: 'rgba(74,168,216,0.25)',    label: 'P25–P75' },
                              { color: '#4aa8d8', dashed: false, fill: 'rgba(74,168,216,0.12)',    label: 'P10–P90' },
                            ].map(({ color, dashed, fill, label }) => (
                        ...
                        {expandCtrPc2 && (
                          <div style={{ padding: '0 10px 8px', borderTop: '1px solid #c0daf0', lineHeight: 1.7 }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2px 10px' }}>
                              {[
                                { color: '#1a4a7a', dashed: false, fill: null,                    label: 'Actual solar round length', value: ctrPc2?.lar },
        ...
        <div style={styles.card}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 2 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: '#1e3a12' }}>Solar radiation (MJ/m²/day)</div>
          ...
          {fTemp && (
            <FormulaBox
              ...
              vars={[
                { label: 'Max solar this month', value: `${maxSolarMonth} MJ/m²/day` },
                { label: 'Radiation today',      value: radiationToday   != null ? `${radiationToday.toFixed(1)} MJ/m²/day` : '—' },
                { label: 'Solar factor today',   value: solarFactorToday != null ? solarFactorToday.toFixed(3) : '—' },
              ]}
            />
          )}
          {pillRow}
          ...
          {!loading && (
            <>
              ...
              {showRaw2 && (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 6, marginTop: 4 }}>
                  {[
                    { key: 'tMean', label: 'Radiation', color: '#c47a12' },
                  ].map(({ key, label, color }) => (
                    <button key={key}
              ...
              {ctr2 && (
                <div style={{ fontSize: 11, color: '#2d4a1e', textAlign: 'center', marginTop: 6, background: '#f5fae8', border: '1px solid #cfe2b3', borderRadius: 6, padding: '6px 9px', lineHeight: 1.5 }}>
                  <strong>{ctr2.dl}</strong> · Radiation <strong>{ctr2.tMean}</strong>
                ...
                {showTpc && (() => { const c = TPC_COL[tpcMetric]; return (
                  <div style={{ padding: '0 12px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, paddingTop: 10, flexWrap: 'wrap' }}>
                      {['mean'].map(mk => { const cc = TPC_COL[mk]; return (
